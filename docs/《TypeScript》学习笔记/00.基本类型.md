...
可以手动设置枚举类型中值的索引从哪个数字开始，默认是从0开始。
...
> 为了兼容没有使用TypeScript的一些库和应用场景，可以使用any退出类型检查
...
虽然这里`strictlyTyped`确实是一个`number`类型，并且确实有`toFixed`方法，但是unknow会检查类型，unkonwn没有toFixed方法。
...
> 表示一个函数总是会抛出一个错误或者永远返回，此时就会返回never类型
