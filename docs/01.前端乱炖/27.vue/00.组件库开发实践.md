...
```
这样我们已经实现了一个最简单的`Button`组件，效果如下：
...
但是样式肯定是不满足我们的要求的，回到`src/theme`目录，目录结构如图：
```vue
├── common
│   └── var.scss
├── config
...
├── index.scss
```
这里结合了[scss](https://www.sass.hk/)和[BEM规范](https://developer.mozilla.org/zh-CN/docs/Learn/CSS/Building_blocks/Organizing)来书写样式。

至于为什么要用`scss`和`BEM规范`来组织样式代码，其实最主要的目的就是便于维护。css的样式是全局的，没有作用域之说，这样造成的后果是：
- 样式代码很难维护 开发者定义样式名的习惯是不固定的。
- 容易出现样式覆盖问题。

可能有人说，每个人都保证样式命名是独一无二的，不就没问题了吗？理论上是这样的，但这是不可靠的，规范往往比人更可靠。`scss`扩展了传统的css，使得css有了作用域，同时也提供了函数、混入等等编程语法特性，解决了样式覆盖的问题。`BEM`则人为的指定了一套样式命名的规范，便于样式代码的维护和开发。
