...
> 浅拷贝和深拷贝对于属性值是基本类型的值的处理都是一样的，将属性拷贝一份到克隆的对象中；但是对于引用类型的属性值，**浅拷贝只拷贝引用地址**，所以克隆的对象和被克隆的对象始终指向同一个内存地址，改变其中一个对象的引用类型的属性值，两者都会变化；**深拷贝则是拷贝整个对象**，内存中会单独开辟一个空间给克隆的对象，修改其中一个对象的值，对另外一个对象没有影响。
...
这里的处理方式就是**浅拷贝**，为什么是浅拷贝呢？因为我们只拷贝了第一级的属性，对于基本类型，本来就只有一级；对于上面例子中的引用类型，我们只克隆了a这个引用，a中的b引用指向的内存地址还是之前的对象中的引用b指向的地址，所以当我们给a.b重新赋值时，obj.a.b和cloneObj.a.b都会是我们新赋值的值

### 二、浅拷贝的实现

#### 1.直接简单粗暴的遍历对象的每一个属性，将遍历到的属性放入创建的新对象中：
...
但是，值得注意的是虽然使用JSON.stringify和JSON.parse不适合下列情况：

JSON.stringify克隆时会忽略属性为undefined、symbol、函数三种情况
...
所以使用**JSON.stringify**和**JSON.parse**还是有很多局限的地方
...
这里稍微解释下属性**b: {c:{d:4,g:[1]},m: 7}**递归的过程，每一次递归都会调用simpleDeepClone这个函数，当函数在嵌套调用时，会发生这样的事情:
...
利用解决循环引用的思路，就是在赋值之前判断当前值是否已经存在，避免循环引用，这里我们可以使用es6的WeakMap来生成一个hash表
...
值得注意的是(1)处，hash.set(obj,target)中的target其实是undefined,但是target在(2)处被赋值，target指向同一个内存地址，hash.set(obj,target)中的target值也改变。
...
```js
对Date对象，正则、函数的兼容function simpleDeepClone(obj, hash = new WeakMap()) {
  let target = null;
  if (obj === null) return null;
  if (hash.has(obj)) return hash.get(obj); // 解决循环引用，hash表中有的直接返回改对象
