...
除了常规的数字外，还有**Infinity**和**NaN**两个特殊的值

> Infinity表示无穷大，NaN表示一个不正确的或者一个未定义的数学操作所得到的结果,**NaN是粘性的，任何对NaN的操作，其结果都将是NaN**
...
值得注意的是**NaN是JavaScript中唯一一个不等于本身的值**，判断一个值是不是NaN,可以这样:
...
JavaScript中有一个方法也可以判断这个值是不是NaN:

```js
console.log(isNaN(m)) // NaN值返回true，否则返回false
```

Number的number的包装器，或者叫包装对象
...
看起来，调用了toString()这个方法，但是num是一个简单类型，javaScript中只有对象才具备方法。其实上述的代码相当于下面这段代码：
...
既然Numbr是一个包装对象，自然具备一些有用的方法,可以在MDN上看具体都有哪些方法。

### 二、BigInt

> BigInt的出现源于javaScript中number只能表示 **-9007199254740991 (-(2^53-1))` 和`9007199254740991(2^53-1)**，超过这个范围的数字就会出现误差，而Binint可以表示任意大的数字

可以在一个数字后面加n的方式来表示这是一个BigInt类型的数字
...
但是当我们想使用new Number一样使用new BigInt的时候会报错
...
那么对于new 和new Number，BigInt和new Number，这有什么区别呢？
...
知道了new 关键字做了什么事情，我们自己试着实现一个简单的new 关键字
...
这里我们确实实现了一个比较粗糙的new关键字，但是对于理解new的原理来说这样就足够了

回到new Number和 number这个问题:
...
使用了new关键字的numObj是object 所以可以使用toString()方法；num也可以使用toString()方法，相当于这样：
...
可以看到，当调用toString()方法时，都会经过包装对象的封装，这是解析器默认做的。

**值得注意的是 BigInt类型不支持 new 关键字示例化对象；**

并且虽然BigInt可以表示任意大的数字，但是为了区别于Number类型，有如下几个特点
...
> JavaScrip中没有类似的别的语言的单个字符表示的类型，如char，所有字符逗包裹在单引号或双引号中
>
> 内部采用UTF-16内部格式存贮
...
**`includes`有时需要我们`polyfill`**，因为未必在所有环境中都能使用，可以这样做
...
其实对于布尔类型，我们只需要明确几个原则，就可判断其值是true还是false

> - 值`0`，`-0`，`null`，`false`，`NaN`，`undefined`)，或空字符串（`""`）,值为false
> - 任何对象，空数组（`[]`）或字符串`"false"`，值都为true
...
> JavaScript中的null是空、无值的意思，别的语言的null一般表示空指针

```js
let name = null. // 表示初始值是空
```

这里需要注意一点是：使用typeof值检测null的类型，并不是null 而是object
...
object是引用类型，对象在JavaSCript中特别重要，单独开篇文章记录，这里只用知道是储存更复杂的数据类型的键值对就行了
